...
FROM country_club.Facilities f
WHERE f.membercost < .2 * f.monthlymaintenance 
...
WHERE m.joindate = (
    SELECT MAX(m.joindate) FROM country_club.Members m
	)
...
the member name. */
SELECT sub.facility,
	   CONCAT(sub.firstname," ",sub.surname) AS member
FROM(
...
GROUP BY sub.facility, sub.firstname, sub.surname
ORDER BY member
